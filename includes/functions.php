...
function rsaacpptelx_init_session()
...
add_action('init', 'rsaacpptelx_init_session');
...
?>
            ...
            <div class="notice notice-error is-dismissible">
                ...
                <p><strong>Recent Sales and Add to Cart Pop Up - Trust Elixir Plugin requires the WooCommerce plugin to be installed and active. You can download <a href="https://wordpress.org/plugins/woocommerce/" target="_blank">WooCommerce</a> here.</strong></p>
...
if (!function_exists('rsaacpptelx_success_message')) {
...

    {
        ...
        if ($_GET['page'] == 'trust-elixir' && $_GET['status'] == 1) { ?>
...
if (!function_exists('rsaacpptelx_email_invalid_error')) {
...

    {
        ...
        if ($_GET['page'] == 'trust-elixir' && $_GET['error'] == 1) {
...
if (!function_exists('rsaacpptelx_add_custom_admin_menu')) {
...

    {
        ...
        add_menu_page('Recent Sales and Add to Cart Pop Up - Trust Elixir', 'Recent Sales and Add to Cart Pop Up - Trust Elixir', 'level_10', $slug, 'rsaacpptelx_order_form');
...
add_action('wp_footer', 'rsaacpptelx_frontend_script');



//function for show popup notification in front end

if (!function_exists('rsaacpptelx_order_form')) {

    function rsaacpptelx_order_form()
...
if (!function_exists('rsaacpptelx_showpopup_initshowpopup')) {
...
if (!function_exists('rsaacpptelx_showpopup')) {
...

    {
        ...
        rsaacpptelx_update_product_view($page_id);

        rsaacpptelx_create_reportdb();
...
if (!function_exists('rsaacpptelx_create_reportdb')) {

    function rsaacpptelx_create_reportdb()
...
if (!function_exists('rsaacpptelx_track_popup_product_click')) {

    function rsaacpptelx_track_popup_product_click($popup_name, $product_id)
...
if (!function_exists('rsaacpptelx_get_track_popup_view')) {

    function rsaacpptelx_get_track_popup_view()
...
if (!function_exists('rsaacpptelx_get_most_view_popup_product')) {

    function rsaacpptelx_get_most_view_popup_product($start_date, $end_date)
...
if (!function_exists('rsaacpptelx_get_last_order_id')) {

    function rsaacpptelx_get_last_order_id()
...
if (!function_exists('rsaacpptelx_update_product_view')) {

    function rsaacpptelx_update_product_view($productid)

    {
        ...
        if (!empty($product)) {
            ...
            rsaacpptelx_product_view_counter($product_id);
...
if (!function_exists('rsaacpptelx_product_view_counter')) {

    function rsaacpptelx_product_view_counter($product_id)
...
if (!function_exists('rsaacpptelx_show_product_view_counter_on_product_page')) {

    function rsaacpptelx_show_product_view_counter_on_product_page($product_id)
...
if (!function_exists('rsaacpptelx_get_customer_recent_order')) {

    function rsaacpptelx_get_customer_recent_order($beforenum = "", $beforedays = "", $excludearray = array(), $ordersts = array())
...
if (!function_exists('rsaacpptelx_get_mostviwed_product')) {

    function rsaacpptelx_get_mostviwed_product()

    {
        ...
        while ($loop->have_posts()) : $loop->the_post();
            ...
            $mostview = rsaacpptelx_show_product_view_counter_on_product_page(get_the_id());
...
if (!function_exists('rsaacpptelx_get_orders_status')) {

    function rsaacpptelx_get_orders_status()
...
if (!function_exists('rsaacpptelx_get_back_images')) {

    function rsaacpptelx_get_back_images()
...
if (!function_exists('rsaacpptelx_get_location_detais')) {
...
if (!function_exists('rsaacpptelx_update_popup_option')) {
    ...
    function rsaacpptelx_update_popup_option($post_data)
    {
        ...
        //     }
        // } else {
        //     $salesproduct = $post_data['salesproduct'];
        // }
        $salesproduct = $post_data['salesproduct'];
        ...
        $orders_sts = $post_data['get_orders_sts'];
        $exclude_product_sts = $post_data['exclude_product_sts'];
        ...
        if (isset($post_data['image_redirect'])) {

            $imgredirect = $post_data['image_redirect'];
        ...
        if (isset($post_data['pro_link_target'])) {

            $pro_link_target = $post_data['pro_link_target'];
        ...
        update_option('image_redirect', $imgredirect, '', 'yes');
        update_option('pro_link_target', $pro_link_target, '', 'yes');
...
if (!function_exists('rsaacpptelx_sales_option_values')) {
...
if (!function_exists('rsaacpptelx_special_char_validation')) {

    function rsaacpptelx_special_char_validation($parms)

    {

        if (preg_match("/^[a-z0-9\s\-_]+$/i", $parms)) {
...
if (!function_exists('rsaacpptelx_number_val_validation')) {

    function rsaacpptelx_number_val_validation($parms)

    {

        if (preg_match('/^([0-9]+)$/', $parms)) {
